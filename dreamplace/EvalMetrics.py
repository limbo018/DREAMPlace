...
class EvalMetrics (object):
    ...
    def __str__(self):
        ...
        if self.shpwl is not None:
            content += ", sHPWL %.6E" % (self.shpwl)
    ...
    def evaluate(self, placedb, ops, var):
        ...
        with torch.no_grad():
            ...
            if "shpwl" in ops:
                hpwl = self.hpwl if "hpwl" in ops else ops["hpwl"](var).data
                overflow = self.overflow if "overflow" in ops else (ops["overflow"](var)[0] / placedb.total_movable_node_area)
                self.shpwl = hpwl * (1 + overflow)

    ...
    at::Tensor inv_gamma, int num_threads)
{
    ...
    at::Tensor xy_max = at::full({2, num_nets}, std::numeric_limits<V>::min(), at::kInt);
...
{
    m.def("forward", &DREAMPLACE_NAMESPACE::weighted_average_wirelength_atomic_forward, "WeightedAverageWirelength forward");
    m.def("backward", &DREAMPLACE_NAMESPACE::weighted_average_wirelength_atomic_backward, "WeightedAverageWirelength backward");
}

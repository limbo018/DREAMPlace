...
class WeightedAverageWirelengthAtomicFunction(Function):
    ...
    """
    @staticmethod
    def forward(ctx, pos, pin2net_map, net_weights, net_mask, pin_mask, inv_gamma):
        """
        @param pos pin location (x array, y array), not cell location 
        @param pin2net_map pin2net map 
        @param net_weights weight of nets 
        ...
        #tt = time.time()
        if pos.is_cuda:
            output = weighted_average_wirelength_cuda_atomic.forward(pos.view(pos.numel()), pin2net_map, net_weights, net_mask, inv_gamma)
        else:

...
class WeightedAverageWirelengthFunction(Function):
    ...
    """
    @staticmethod
    def forward(ctx, pos, flat_netpin, netpin_start, pin2net_map, net_mask, pin_mask, net_weights, gamma, num_threads):
        """
        ...
        """
        if pos.is_cuda:
            output = weighted_average_wirelength_cuda.forward(pos.view(pos.numel()), flat_netpin, netpin_start, pin2net_map, net_mask, net_weights, gamma)
        else:
            output = weighted_average_wirelength_cpp.forward(pos.view(pos.numel()), flat_netpin, netpin_start, net_mask, net_weights, gamma, num_threads)
...
class WeightedAverageWirelengthAtomicFunction(Function):
    ...
    """
    @staticmethod
    def forward(ctx, pos, pin2net_map, net_mask, pin_mask, net_weights, gamma):
        """
        @param pos pin location (x array, y array), not cell location 
        @param pin2net_map pin2net map 
        ...
        #tt = time.time()
        if pos.is_cuda:
            output = weighted_average_wirelength_cuda_atomic.forward(pos.view(pos.numel()), pin2net_map, net_mask, net_weights, gamma)
        else:
...
class WeightedAverageWirelengthSparseFunction(Function):
    ...
    """
    @staticmethod
    def forward(ctx, pos, flat_netpin, netpin_start, netpin_values, pin2net_map, net_mask, pin_mask, net_weights, gamma):
        """
        @param pos pin location (x array, y array), not cell location 
        @param pin2net_map pin2net map 
        @param net_mask whether to compute wirelength 
        ...
        #tt = time.time()
        if pos.is_cuda:
            output = weighted_average_wirelength_cuda_sparse.forward(pos.view(pos.numel()), flat_netpin, netpin_start, netpin_values, pin2net_map, net_mask, net_weights, gamma)
        else:
            assert 0, "CPU version NOT IMPLEMENTED"
        ctx.pin2net_map = pin2net_map 
        ctx.net_mask = net_mask 
...
class WeightedAverageWirelength(nn.Module):
    ...
    """
    def __init__(self, flat_netpin=None, netpin_start=None, pin2net_map=None, net_mask=None, pin_mask=None, net_weights=None, gamma=None, algorithm='atomic', num_threads=8):
        """
        ...
        assert net_mask is not None \
                ...
                and gamma is not None, "net_mask, pin_mask, net_weights, gamma are requried parameters"

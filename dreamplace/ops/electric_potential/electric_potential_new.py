...
class ElectricPotentialFunction(Function):
    ...
    def forward(
        ...
        # TODO integrate the following calculations into the dct2_fft2 kernel
        # wu and wv
        ...
        # TODO use one function to generate auv_by_wu2_plus_wv2_wu auv_by_wu2_plus_wv2_wv auv_by_wu2_plus_wv2(not useful in fast mode)
        ...
        if fast_mode:  # dummy for invoking backward propagation
            ...
            # auv / (wu**2 + wv**2)
            auv_by_wu2_plus_wv2 = auv.mul(inv_wu2_plus_wv2_2X).mul_(2) # TODO scale factor
...
class ElectricPotential(nn.Module):
    ...
    def forward(self, pos):
        if self.initial_density_map is None:
            ...
            wv = torch.arange(N, dtype=pos.dtype, device=pos.device).mul(
                2*np.pi/N).view([1, N]).mul_(self.bin_size_x/self.bin_size_y)
            wu2_plus_wv2 = wu.pow(2) + wv.pow(2)
            wu2_plus_wv2[0, 0] = 1.0  # avoid zero-division, it will be zeroed out

        ...
        )
{
    ...
    if (state.target_costs[i] < state.orig_costs[i])
    {
        ...
        //        printf("node %d(%d) net %d(%d)\n", j, node_id, net_id, db.net_mask[net_id]);
        ...
        //                typename DetailedPlaceDBType::type xxl = db.x[other_node_id];
        //                typename DetailedPlaceDBType::type yyl = db.y[other_node_id];
        //                bxl = std::min(bxl, xxl+db.pin_offset_x[net_pin_id]);
        //                bxh = std::max(bxh, xxl+db.pin_offset_x[net_pin_id]);
        //                byl = std::min(byl, yyl+db.pin_offset_y[net_pin_id]);
        //                byh = std::max(byh, yyl+db.pin_offset_y[net_pin_id]);
        //            }
        //            target_hpwl += (bxh-bxl) + (byh-byl); 
        //        }
        //    }
        //    printf("[D] node %d(%d) original hpwl %g\n", j, node_id, target_hpwl);
        //}
#endif
        // move cells 
        int count = 0; 
...
//#endif
...
#ifdef DEBUG
                //printf("[D] move node %d(%d) pos %d(%d): (%g, %g) => (%g, %g), displace (%g, %g)\n", 
                //        j, target_node_id, sol_j, target_pos_id, 
                //        db.x[target_node_id], db.y[target_node_id], 
                //        target_pos_x[sol_j], target_pos_y[sol_j], 
                //        target_pos_x[sol_j]-db.x[target_node_id], target_pos_y[sol_j]-db.y[target_node_id] 
                //      );
                ...
                if (!(target_pos_x[sol_j] >= target_spaces[sol_j].xl && target_pos_x[sol_j]+db.node_size_x[target_node_id] <= target_spaces[sol_j].xh))
                {
                    printf("node %d, %g, %g, pos %d, %g, %g, space %g, %g\n", 
                            target_node_id, db.x[target_node_id], db.x[target_node_id]+db.node_size_x[target_node_id], 
                            target_pos_id, target_pos_x[sol_j], target_pos_x[sol_j]+db.node_size_x[target_pos_id], 
...
#pragma omp atomic
        ...
        //        }
        //    }
        //    printf("[D] node %d(%d) target hpwl %g\n", j, node_id, target_hpwl);
        //}
        //printf("[D] original cost %g, target cost %g, delta %g\n", 
        //        state.orig_costs[i], state.target_costs[i], state.target_costs[i]-state.orig_costs[i]);
        //printf("[D] %d/%d: original hpwl %g, target hpwl %g, delta %g(%g%%), cumulated delta %g%%, moved %d/%lu, cumulated moved %g%%\n", 
        //        state.cur_idx, db.num_movable_nodes, 
        //        orig_hpwl, target_hpwl, target_hpwl-orig_hpwl, (target_hpwl-orig_hpwl)/orig_hpwl*100, 
        //        (target_hpwl-initial_hpwl)/initial_hpwl*100, 
        //        count, independent_set.size(), state.num_moved/(double)db.num_movable_nodes*100);
        //dreamplaceAssert(target_hpwl-orig_hpwl == state.target_costs[i]-state.orig_costs[i]);

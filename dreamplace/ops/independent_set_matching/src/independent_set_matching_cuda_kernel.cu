...
struct IndependentSetMatchingState
{
    ...
    unsigned int* ordered_node_keys = nullptr; 
        ...
        int batch_size, int set_size, int max_iters)
{
    ...
    timer_start = get_globaltime();
    {
        ...
        checkCURAND(curandCreateGenerator(&gen, 
                    CURAND_RNG_PSEUDO_DEFAULT));

        /* Set seed */
        checkCURAND(curandSetPseudoRandomGeneratorSeed(gen, 
                    1234ULL));
        ...
        allocateCUDA(state.ordered_node_keys, db.num_movable_nodes, unsigned int);
    ...
    for (int iter = 0; iter < max_iters; ++iter)
    {
        ...
        shuffle(gen, state.ordered_nodes, state.ordered_node_keys, db.num_movable_nodes);
    ...
    printf("[D] random_shuffle takes %g ms, %d runs, average %g ms\n", 
            get_timer_period()*random_shuffle_time, random_shuffle_runs, get_timer_period()*random_shuffle_time/random_shuffle_runs);
    printf("[D] maximum_independent_set takes %g ms, %d runs, average %g ms\n", 
    ...
    printf("[D] collect_independent_sets takes %g ms, %d runs, average %g ms\n", 
    ...
    printf("[D] cost_matrix_construction takes %g ms, %d runs, average %g ms\n", 
    ...
    printf("[D] independent_sets_solving takes %g ms, %d runs, average %g ms\n", 
    ...
    printf("[D] apply_solution takes %g ms, %d runs, average %g ms\n", 
    ...
    timer_start = get_globaltime();
    {
        ...
        destroyCUDA(state.ordered_node_keys);

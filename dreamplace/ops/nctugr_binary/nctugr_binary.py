...
class NCTUgr(object):
    def __init__(self, 
            ...
            tmp_pl_file, 
            tmp_output_file, 
            horizontal_routing_capacities, 
            vertical_routing_capacities, 
            params, 
            placedb
            ):
        ...
        self.routing_capacities = (horizontal_routing_capacities + vertical_routing_capacities).view([1, 1, len(horizontal_routing_capacities)])
        self.params = params 
    ...
    def forward(self, pos):
        ...
        place_io.PlaceIOFunction.write(self.placedb.rawdb, self.tmp_pl_file, place_io.SolutionFileFormat.BOOKSHELF, pos_cpu[:num_nodes], pos_cpu[num_nodes:])
        ...
        cmd = "ln -s %s/PORT9.dat .; \
               ...
               ln -s %s/POWV9.dat .; \
                ...
                " % (self.nctugr_dir, self.nctugr_dir, self.nctugr_dir, self.nctugr_dir, 
                        self.aux_input_file, self.tmp_pl_file, self.param_setting_file, self.tmp_output_file)
        logger.info(cmd)
        os.system(cmd)

        congestion_map = torch.zeros((self.placedb.num_routing_grids_x, self.placedb.num_routing_grids_y, self.placedb.num_routing_layers), dtype=pos.dtype)
        with open(self.tmp_output_file + ".ofinfo", "r") as f:
            ...
            for line in f:
                ...
                if line.startswith("Overflowed grid edges :"):
                    ...
                    tokens = line.split()
                    start = (int(tokens[0][1:-1]), int(tokens[1][:-1]), int(tokens[2][:-1]))
                    end = (int(tokens[4][1:-1]), int(tokens[5][:-1]), int(tokens[6][:-1]))
        ...
        overflow_map = congestion_map.to(pos.device) / (self.routing_capacities + 1e-6) + 1

...
class LogSumExpWirelengthMergedFunction(Function):
    ...
    def forward(ctx, pos, flat_netpin, netpin_start, pin2net_map, net_weights, net_mask, pin_mask, gamma, num_threads):
        ...
        tt = time.time()
        if pos.is_cuda:
            output = logsumexp_wirelength_cuda_merged.forward(pos.view(pos.numel()), flat_netpin, netpin_start, pin2net_map, net_weights, net_mask, gamma)
        else:
            output = logsumexp_wirelength_cpp_merged.forward(pos.view(pos.numel()), flat_netpin, netpin_start, pin2net_map, net_weights, net_mask, gamma, num_threads)
    ...
    def backward(ctx, grad_pos):
        tt = time.time()
        if grad_pos.is_cuda:
            output = logsumexp_wirelength_cuda_merged.backward(
                    grad_pos,
                    ctx.pos,
            ...
            output = logsumexp_wirelength_cpp_merged.backward(
                    grad_pos,
                    ctx.pos,
                    ctx.grad_intermediate,
                    ctx.flat_netpin,
...
class LogSumExpWirelength(nn.Module):
    ...
    """
    def __init__(self, flat_netpin=None, netpin_start=None, pin2net_map=None, net_weights=None, net_mask=None, pin_mask=None, gamma=None, algorithm='merged', num_threads=8):
    ...
    def forward(self, pos): 
        if pos.is_cuda:
            if self.algorithm == 'atomic':
                ...
                return LogSumExpWirelengthMergedFunction.apply(pos,
                        self.flat_netpin,
                        self.netpin_start,
                        self.pin2net_map,
                        self.net_weights,

...
class LogSumExpWirelengthFunction(Function):
    ...
    """
    @staticmethod
    def forward(ctx, pos, flat_netpin, netpin_start, netpin_values, pin2net_map, net_weights, net_mask, gamma, num_threads):
        if pos.is_cuda:
            output = logsumexp_wirelength_cuda.forward(pos.view(pos.numel()), flat_netpin, netpin_start, netpin_values, pin2net_map, net_weights, net_mask, gamma)
        else:
            output = logsumexp_wirelength_cpp.forward(pos.view(pos.numel()), flat_netpin, netpin_start, net_weights, net_mask, gamma, num_threads)
...
class LogSumExpWirelength(nn.Module):
    ...
    """
    def __init__(self, flat_netpin=None, netpin_start=None, pin2net_map=None, net_weights=None, net_mask=None, gamma=None, algorithm='atomic', num_threads=8):
        super(LogSumExpWirelength, self).__init__()
        assert net_weights is not None \
                and net_mask is not None \
                and gamma is not None, "net_weights, net_mask, gamma are requried parameters"

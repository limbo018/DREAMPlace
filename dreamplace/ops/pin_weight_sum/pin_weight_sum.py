...
class PinWeightSumFunction(Function):
    ...
    """
    @staticmethod
    def forward(ctx, net_weights, flat_nodepin, nodepin_start, pin2net_map):
        if net_weights.is_cuda:
            func = pws_cuda.forward
        else:
            func = pws_cpp.forward
        output = func(net_weights, flat_nodepin, nodepin_start, pin2net_map)
        return output
    ...
    def forward(self, pos):
        if self.algorithm == 'node-by-node':
            return PinWeightSumFunction.apply(
                self.net_weights, self.flat_nodepin, self.nodepin_start,
                self.pin2net_map)

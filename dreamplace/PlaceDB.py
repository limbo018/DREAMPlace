...
class PlaceDB (object):
    ...
    def calc_num_filler_for_fence_region(self, fence_regions, region_id, node2fence_region_map, target_density):
        ...
        if(region_id < num_regions):
            ...
            # placeable_area2 = fence_region.calc_region_area(region)
            ...
            # fence_regions_size_x = np.ceil(fence_regions_size_x / self.bin_size_x)*self.bin_size_x
            # fence_regions_size_y = np.ceil(fence_regions_size_y / self.bin_size_y)*self.bin_size_y
            fence_region_area = np.sum(fence_regions_size_x*fence_regions_size_y)

            # fence_region_area2 = fence_region.calc_region_area(fence_regions)
            # print(fence_region_area, fence_region_area2)

...
class PreconditionOp:
    ...
    def __call__(self, grad, density_weight):
        ...
        """
        with torch.no_grad():
            precond = self.data_collections.num_pins_in_nodes + \
            ...
            # Jiaqi: may cause divergence and NaN, so removed
...
class PlaceObj(nn.Module):
    ...
    def __init__(self, density_weight, params, placedb, data_collections, op_collections, global_place_params):
        ...
        self.gamma = torch.tensor(10*self.base_gamma(params, placedb),
                                  dtype=self.data_collections.pos[0].dtype, device=self.data_collections.pos[0].device)
        ...
        self.op_collections.density_op = self.build_electric_potential(
            params, placedb, self.data_collections, global_place_params["num_bins_x"], global_place_params["num_bins_y"], padding=0, name=name)
        self.op_collections.electric_potential_overflow_op = self.build_electric_potential_overflow(
            params, placedb, self.data_collections, global_place_params["num_bins_x"], global_place_params["num_bins_y"], padding=0, name=name)
        ...
        self.op_collections.align_and_overflow_op = self.build_align_and_overflow(
            params, placedb)
    ...
    def obj_fn(self, pos):
        ...
        if(self.quad_penalty):
            ...
            return wirelength + self.density_weight * (density + self.density_quad_coeff / 2 / self.init_density * density**2) * self.density_factor

        else:
            return wirelength + self.density_weight * density

    def obj_fn_2(self, pos):
        """
        @brief Compute objective.
            wirelength + density_weight * density overflow penalty
        @param pos locations of cells
        @return objective value
        """
        wirelength = self.op_collections.wirelength_op(pos)
        density = self.op_collections.electric_potential_overflow_op(pos)
    ...
    def build_density_potential(self, params, placedb, data_collections, num_bins_x, num_bins_y, padding, name):
        ...
        logging.info("%s #bins %dx%d, bin sizes %gx%g, max_num_bins = %d, padding = %d" % (name, local_num_bins_x,
                                                                                           local_num_bins_y, bin_size_x/placedb.row_height, bin_size_y/placedb.row_height, max_num_bins, padding))
        ...
        # coefficients
        ax = (4 / (node_size_x + 2*bin_size_x) / (node_size_x + 4*bin_size_x)
              ).astype(placedb.dtype).reshape([placedb.num_nodes, 1])
        bx = (2 / bin_size_x / (node_size_x + 4*bin_size_x)
              ).astype(placedb.dtype).reshape([placedb.num_nodes, 1])
        ay = (4 / (node_size_y + 2*bin_size_y) / (node_size_y + 4*bin_size_y)
              ).astype(placedb.dtype).reshape([placedb.num_nodes, 1])
        by = (2 / bin_size_y / (node_size_y + 4*bin_size_y)
              ).astype(placedb.dtype).reshape([placedb.num_nodes, 1])
        ...
        integral_potential_x = npfx1(
            0) + 2*npfx1(bin_size_x) + 2*npfx2(2*bin_size_x)
        cx = (node_size_x.reshape([placedb.num_nodes, 1]) /
              integral_potential_x).reshape([placedb.num_nodes, 1])
        # should not use integral, but sum; basically sample 5 distances, -2wb, -wb, 0, wb, 2wb; the sum does not change much when shifting cells
        integral_potential_y = npfy1(
            0) + 2*npfy1(bin_size_y) + 2*npfy2(2*bin_size_y)
    ...
    def build_electric_potential(self, params, placedb, data_collections, num_bins_x, num_bins_y, padding, name):
        ...
        local_num_bins_x = num_bins_x + 2*padding
        local_num_bins_y = num_bins_y + 2*padding
        max_num_bins_x = np.ceil((np.amax(
            placedb.node_size_x[0:placedb.num_movable_nodes])+2*bin_size_x) / bin_size_x)
        max_num_bins_y = np.ceil((np.amax(
            placedb.node_size_y[0:placedb.num_movable_nodes])+2*bin_size_y) / bin_size_y)
        max_num_bins = max(int(max_num_bins_x), int(max_num_bins_y))
        logging.info("%s #bins %dx%d, bin sizes %gx%g, max_num_bins = %d, padding = %d" % (name, local_num_bins_x,
                                                                                           local_num_bins_y, bin_size_x/placedb.row_height, bin_size_y/placedb.row_height, max_num_bins, padding))
        if local_num_bins_x < max_num_bins:
    ...
    def build_electric_potential_overflow(self, params, placedb, data_collections, num_bins_x, num_bins_y, padding, name):
        """
        @brief e-place electrostatic potential overflow
        @param params parameters
        @param placedb placement database
        @param data_collections a collection of data and variables required for constructing ops
        @param num_bins_x number of bins in horizontal direction
        ...
        return electric_potential.ElectricPotentialOverflow(
            node_size_x=data_collections.node_size_x, node_size_y=data_collections.node_size_y,
            bin_center_x=data_collections.bin_center_x_padded(placedb, padding), bin_center_y=data_collections.bin_center_y_padded(placedb, padding),
            target_density=data_collections.target_density,
            xl=xl, yl=yl, xh=xh, yh=yh,
    ...
    def build_update_density_weight(self, params, placedb):
        ...
        def update_density_weight_op(cur_metric, prev_metric, iteration):
            with torch.no_grad():
                ...
                if delta_hpwl < 0:
                    mu = UPPER_PCOF * \
                        np.maximum(np.power(0.9999, float(iteration)), 0.98)
                    ...
                    if(self.quad_penalty):
                        mu = (UPPER_PCOF*torch.pow(UPPER_PCOF, -delta_hpwl/ref_hpwl)
                              ).clamp(min=LOWER_PCOF*1.05, max=UPPER_PCOF*1.1)
                    else:
                        mu = (UPPER_PCOF*torch.pow(UPPER_PCOF, -delta_hpwl /
                                                   ref_hpwl)).clamp(min=LOWER_PCOF, max=UPPER_PCOF)
        ...
        def update_density_weight_elfplace_op(cur_metric, prev_metric, iteration):
            with torch.no_grad():
                ...
                self.density_step_size *= np.log(self.density_quad_coeff * density_norm + 1) / (
                    1+np.log(self.density_quad_coeff * density_norm + 1)) * (alpha_h-alpha_l) + alpha_l
    ...
    def build_noise(self, params, placedb, data_collections):
        ...
        def noise_op(pos, noise_ratio):
            with torch.no_grad():
                ...
                # noise[:placedb.num_nodes] = (noise[:placedb.num_nodes] - 0.5) * (placedb.xh - placedb.xl) / 1
                # noise[placedb.num_nodes:] = (noise[placedb.num_nodes:] - 0.5) * (placedb.yh - placedb.yl) / 1
                ...
                noise[placedb.num_nodes+placedb.num_movable_nodes:2 *
                      placedb.num_nodes-placedb.num_filler_nodes].zero_()
    ...
    def build_nctugr_congestion_map(self, params, placedb, data_collections):
        ...
        return nctugr_binary.NCTUgr(
            ...
            tmp_pl_file="%s/%s.NCTUgr.pl" % (os.path.realpath(path),
                                             params.design_name()),
    ...
    def build_adjust_node_area(self, params, placedb, data_collections):
        ...
        """
        total_movable_area = (data_collections.node_size_x[:placedb.num_movable_nodes]
                              * data_collections.node_size_y[:placedb.num_movable_nodes]).sum()
        total_filler_area = (data_collections.node_size_x[-placedb.num_filler_nodes:]
                             * data_collections.node_size_y[-placedb.num_filler_nodes:]).sum()
        total_place_area = (total_movable_area +
                            total_filler_area) / data_collections.target_density
    ...
    def build_align_and_overflow(self, params, placedb):
        ...
        def scatter_sum(src, index, dim, dim_size, out=None) -> torch.Tensor:
            ...
            if out is None:
                out = torch.zeros(
                    dim_size+1, dtype=src.dtype, device=src.device)
        ...
        def align_and_overflow(pos):
            ...
            node_size_x, node_size_y = torch.from_numpy(placedb.node_size_x[:num_movable_nodes]).to(
                pos_x.device), torch.from_numpy(placedb.node_size_y[:num_movable_nodes]).to(pos_x.device)

            num_rows = int(round((yh - yl)/row_height))
            # row_capacity = xh - xl

            # center_y = pos_y + node_size_y / 2
            aligned_rows_index = torch.round(
                (pos_y - yl) / row_height).clamp_(0, num_rows-1)
            ...
            overflow = scatter_sum(
                node_size_x, aligned_rows_index, dim=0, dim_size=num_rows)

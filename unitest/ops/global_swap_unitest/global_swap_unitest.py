...
def plot(figname, 
    ...
    def normalize_x(xx):
        return (xx - (layout_xl-bin_size_x))/(layout_xh-layout_xl+2*bin_size_x)*width
    def normalize_y(xx):
        return (xx - (layout_yl-bin_size_y))/(layout_yh-layout_yl+2*bin_size_y)*height
...
class GlobalSwapOpTest(unittest.TestCase):
    def test_random(self):
        ...
        pin2net_map, flat_net2pin_map, flat_net2pin_start_map = flatten_2D_map(net2pin_map)
        node2pin_map = np.array([np.array([0, 1]), np.array([2]), np.array([3, 4])])
        pin2node_map, flat_node2pin_map, flat_node2pin_start_map = flatten_2D_map(node2pin_map)
        # net mask 
        ignore_net_degree = 4
        ...
        custom = global_swap.GlobalSwap(
                    ...
                    torch.from_numpy(flat_net2pin_map), torch.from_numpy(flat_net2pin_start_map), torch.from_numpy(pin2net_map), 
                    torch.from_numpy(flat_node2pin_map), torch.from_numpy(flat_node2pin_start_map), torch.from_numpy(pin2node_map), 
        ...
        custom_cuda = global_swap.GlobalSwap(
                    ...
                    torch.from_numpy(flat_net2pin_map).cuda(), torch.from_numpy(flat_net2pin_start_map).cuda(), torch.from_numpy(pin2net_map).cuda(), 
                    torch.from_numpy(flat_node2pin_map).cuda(), torch.from_numpy(flat_node2pin_start_map).cuda(), torch.from_numpy(pin2node_map).cuda(), 
                    torch.from_numpy(pin_offset_x).cuda(), torch.from_numpy(pin_offset_y).cuda(), 
...
def test_ispd2005(design, algorithm, device_str):
    with gzip.open(design, "rb") as f:
        ...
        # test cpu/cuda  
        custom = global_swap.GlobalSwap(
                    node_size_x=node_size_x.float().to(device), node_size_y=node_size_y.float().to(device), 
                    flat_net2pin_map=flat_net2pin_map.to(device), flat_net2pin_start_map=flat_net2pin_start_map.to(device), pin2net_map=pin2net_map.to(device), 
                    flat_node2pin_map=flat_node2pin_map.to(device), flat_node2pin_start_map=flat_node2pin_start_map.to(device), pin2node_map=pin2node_map.to(device), 
                    pin_offset_x=pin_offset_x.float().to(device), pin_offset_y=pin_offset_y.float().to(device), 
                    net_mask=net_mask.to(device), 
                    xl=xl, yl=yl, xh=xh, yh=yh, 
                    site_width=site_width, row_height=row_height, 
                    num_bins_x=num_bins_x//2, num_bins_y=num_bins_y//2, 
                    num_movable_nodes=num_movable_nodes, 
                    num_filler_nodes=num_filler_nodes, 
                    batch_size=256, 
                    max_iters=2, 
...
if __name__ == '__main__':
    #unittest.main()
    if len(sys.argv) < 4:
        print("usage: python script.py design.pklz sequential|concurrent cpu|cuda")
    else:
        design = sys.argv[1]
